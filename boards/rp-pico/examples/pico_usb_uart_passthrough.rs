...
static mut GLOBAL_UART: Option<Uart> = None;

static mut USB_WRITE_BUF: [u8; 1024] = [0u8; 1024];
...
unsafe fn USBCTRL_IRQ() {
    ...
    let uart = GLOBAL_UART.as_mut().unwrap();
    // let mut buf = USB_WRITE_BUF.as_mut().unwrap();
    ...
    if usb_dev.poll(&mut [serial]) {
        ...
        loop {
            match serial.read(&mut USB_WRITE_BUF) {
                Err(_e) => {
                    // Do nothing
                    break;
                }
                ...
                Ok(count) => {
                    ...
                    USB_WRITE_BUF.iter().take(count).for_each(|b| {
                        uart.write(*b);
                    });
                }
            }
...
unsafe fn UART0_IRQ() {
    ...
    while let Ok(byte) = uart.read() {
        ...
        if ptr + 1 >= buf.len() {
            let mut write_ptr = 0;
            loop {
                let mut data = &buf[write_ptr..ptr];
                let len = serial.write(data);
                if let Ok(len) = len {
                    write_ptr += len;
                    if write_ptr < ptr {
                        continue;
